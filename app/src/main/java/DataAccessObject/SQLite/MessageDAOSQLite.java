...
public class MessageDAOSQLite extends SQLiteOpenHelper implements iMessageDAO {
    ...
    */
        ...
        this.getWritableDatabase().execSQL(
                "create table if not exists message (id integer primary key autoincrement, sent_by varchar(50), sent_to varchar(50),message varchar(255), sent_on integer,server_synchronization integer, FOREIGN KEY(sent_by) REFERENCES contact(identifier), FOREIGN KEY(sent_to) REFERENCES contact(identifier)  )"
        );
    ...
    public ArrayList<Message> getFullConversationWithContact(Contact c) {
        ArrayList<Message> list= new ArrayList<>();
        //String sql="select message,sent_on from message where identifier=?";
        String sql="select m.message,m.sent_on,m.sent_by from message m where m.sent_by=? or m.sent_to=? order by sent_on desc";
        Cursor cursor= this.getReadableDatabase().rawQuery(sql,new String[]{c.getName(),c.getName()});
        while(cursor.moveToNext()) {
            String message=cursor.getString(0);
            Date date=new Date(cursor.getLong(1));
            Message m= new Message(message,date);
            if (cursor.getString(2).equals(c.getName())) {
                m.setSender(c);
            }else{
